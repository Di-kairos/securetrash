...
next_actions:
  - "panic Pack 3 (--hard + физ. диски): прибить cloud-демоны (Dropbox/OneDrive/bird/Google Drive — pkill), почистить recent items (или честно сказать что не можем), опц. размонтировать внешние физ. диски (diskutil unmount). TDD + стабы (pkill/diskutil). После — panic release-prep (install.sh/release.yml/CHANGELOG зеркало vaultwatch) + public+tag"
  - "panic Pack 2 ГОТОВ: ядро 'now' (Di-kairos/panic private, head 01ccf51, bats 15/15, CI green): detach -force всех /Volumes disk image'ов, pbcopy clipboard, CGSession -suspend lock (PANIC_CGSESSION override). Real-device smoke пройден (live hdiutil info parse + detach тест-образа). Без confirm (explicit 'now' verb = защита)"
