...
next_actions:
  - "panic Pack 2 (ядро 'now'): закрыть открытые vault'ы (securetrash vault close по списку / hdiutil detach -force смонтированных sparsebundle), pbcopy </dev/null (clipboard), lock screen (pmset displaysleepnow / osascript). State-detect открытых vault'ов. TDD + PATH-стабы (hdiutil/pbcopy/osascript/pmset/diskutil). --hard в Pack 3"
  - "panic Pack 1 scaffold ГОТОВ: Di-kairos/panic (private), head 5b993a2, bats 9/9, CI green, vendor synced (pin 2e3d2dd). dispatcher version/help + 'now' deferred(exit2). Explicit 'now' verb (анти-случайный-триггер)"
