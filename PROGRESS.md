...
next_actions:
  - "старт ecosystem tool #2 panic (kill-switch: закрыть все vault'ы, hdiutil detach -force, очистить clipboard, lock screen; --hard опц.). Переиспользует vaultwatch close/detach-логику; вендорить common.sh против пиннутого securetrash-ref; отдельный репо Di-kairos/panic"
